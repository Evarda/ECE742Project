...
\end{equation}
 ...
 \begin{equation}
 E_{z}^{n+1}(i,j) = CEZ1  E_{z}^{n}(i,j) + CEZ2  (CEZ3 D_{z}^{n+1}(i,j)-CEZ4 D_{y}^{n}(i,j))/\epsilon
 \end{equation}
 
 Similarly, we can find the B update equations to be:

 \begin{equation}
 B_{x}^{n+1/2}(i,j) = CBX1  B_{x}^{n-1/2}(i,j) + CBX2  (E_{z}^{n}(i,j+1)-E_{z}^{n}(i,j))/dy
 \end{equation}
 \begin{equation}
 B_{y}^{n+1/2}(i,j) = CBY1  B_{y}^{n-1/2}(i,j) + CBY2  (E_{z}^{n}(i+1,j)-E_{z}^{n}(i,j))/dx
 \end{equation}
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{ErrorPlot1}
